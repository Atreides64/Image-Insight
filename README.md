...
individual cards, charts, search/history sections, the file type table, and a
compact dashboard mode, and those preferences are saved in browser
`localStorage` on that device. Scan History is collapsed by default to keep the
main monitoring view dense, with an inline expand/collapse control when recent
scan details are needed.
...
ExifTool, metadata backfill, cancellation, and system-info work. It keeps the
dashboard compact and readable without changing backend scan behavior.
Duplicates, maps, open-in-folder, previews, scan groups, archive scanning,
desktop packaging, cloud integrations, and external job services remain future
work.

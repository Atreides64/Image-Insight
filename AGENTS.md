...
- `/photos/search` is a read-only SQLite query over the existing `photos` table with optional metadata filters for camera, lens, focal length, extension, ISO, aperture, shutter speed, capture date range, and device type plus safe pagination and sortable results.
- `/photos/search` defaults to `limit=50`, caps requested limits at 500, defaults `offset=0`, supports `sort_by`/`sort_order`, and validates date/focal-length/pagination/sort ranges with clean 400 responses.
- `/photos/search-options` returns capped distinct camera, lens, extension, ISO, aperture, shutter speed, and device type values for search dropdowns/autocomplete.

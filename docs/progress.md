...
Image Insight has a FastAPI backend with `/health`, `/system-info`, `POST /scan-folder`, `/scan-status/{scan_id}`, `/scan-sessions`, `/scan-sessions/{scan_id}`, `POST /scan-sessions/{scan_id}/cancel`, `/photos`, `/photos/search`, and `/stats`. `/system-info` returns app version, SQLite database path, indexed photo count, scan session count, and ExifTool detection for lightweight runtime visibility. `POST /scan-folder` starts a lightweight Python-thread background scan job and returns `scan_id` quickly while `/scan-status/{scan_id}` exposes live persisted counters, scan speed, force-metadata flag, ExifTool availability, and elapsed time for polling. `GET /scan-sessions` serves recent scan history with folder path, status, timestamps, elapsed seconds, counters, last error, optional folder filtering, force-metadata flag, ExifTool availability, scan speed, and a safe `limit` parameter. Scan jobs still create durable scan session records in SQLite and support `resume=true` for the latest failed or interrupted scan on the same folder. Resumed scans reuse the same `scan_id`, preserve committed session-file state, skip already committed file work when possible, and still count unchanged files as `skipped_files` rather than `updated_files`. Duplicate running scans for the same folder are rejected unless `resume=true` is attaching to the existing running session. Running scans can be cancelled with an in-memory `POST /scan-sessions/{scan_id}/cancel` request; the scan loop exits early, commits progress, and stores `cancelled` as a terminal status. Scans extract best-effort EXIF metadata for camera make/model, lens model, focal length, ISO, aperture, shutter speed, and capture date while allowing missing or unreadable EXIF to scan cleanly. If `exiftool` is available on PATH, scans use ExifTool JSON metadata first for stronger RAW/DNG/RAF support, then fall back to Pillow for unavailable fields or when ExifTool is not installed. `POST /scan-folder?force_metadata=true` re-runs EXIF extraction for unchanged files and only backfills missing EXIF fields with newly populated values. EXIF focal-length parsing handles Pillow rationals, tuple fractions, numbers, and common string forms, and lens labels fall back from LensModel to LensMake plus LensSpecification when available. `/photos/search` filters the existing SQLite `photos` table by camera model, lens model, focal length range, capture date range, extension, limit, and offset, and returns `total_count` plus paginated results. `/stats` now also includes `photo_timeline`, a monthly date-taken insight series with photo counts plus top camera and lens labels for tooltips. Scans stream directly over the recursive folder walk, upsert records into SQLite at the repo-root `image_insight.db`, commit writes every 500 matched image files, and print progress counters to the terminal during long runs. A lightweight pytest suite covers health, system info, stats, EXIF extraction, mocked ExifTool availability/output, mocked EXIF focal/lens parsing, forced metadata backfill, scan cancellation, scan history response/limit behavior, metadata search filtering, invalid search ranges, background scan start/status polling, duplicate running scan prevention, failed/interrupted resume flows, unchanged-after-resume skip behavior, and timeline insight stats against a temporary SQLite database. The React/Vite dashboard fetches `/system-info` and `/stats`, includes a compact System Info panel with database-path copying, includes a localStorage-backed Customize Dashboard panel for individual cards and sections plus a persisted Compact mode toggle, uses smaller stat cards, displays a richer Recharts photo timeline insight chart when enabled, includes a compact metadata search/filter panel with a results table and copy-path actions, shows clearer empty states, keeps Scan History collapsed by default with an expand/collapse control and rerun/resume actions when expanded, starts scans without holding the request open, polls `/scan-status/{scan_id}` every 2 seconds while running, shows live counters, scan speed, ExifTool/backfill indicators, active scan errors, can cancel an active scan, refreshes stats after completion, offers a Refresh metadata scan checkbox, and offers a calm resume action when the last scan did not complete cleanly.

# Files Changed This Session

- Modified `frontend/src/App.tsx` and `frontend/src/styles.css` to extend v0.9.0 UX polish with persisted Compact mode, denser dashboard spacing, smaller stat/scan/system/search surfaces, and collapsed-by-default Scan History.
...
- Keep v0.9.0 limited to frontend polish and documentation on top of the existing v0.8 scan, ExifTool, backfill, cancellation, and system-info behavior.
- Default Compact mode to on for a denser monitoring dashboard, while keeping it as a browser-local preference alongside the existing dashboard customization controls.
- Keep Scan History visible as a dashboard section preference, but collapse its detailed table by default so recent history does not dominate the main view.
...
- 2026-05-04: Added v0.9.0 stability and UX polish with clearer scan-state summaries, improved empty states, search-result path copying, System Info database-path copying, and README/progress updates.
- 2026-05-04: Extended v0.9.0 dashboard polish with persisted Compact mode, tighter stat/system/scan/search spacing, and collapsed-by-default Scan History with an expand/collapse toggle.

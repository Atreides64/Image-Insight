...
- Modified `app/main.py`, `frontend/src/App.tsx`, and `frontend/src/styles.css` for v1.3.0 sortable Metadata Search, expanded result fields, `/analytics`, Analytics Explorer, result-column preferences, top capture dates, distributions, and focal/shutter buckets.
...
- Keep `/stats` as the default dashboard analytics source, and use `/analytics` for custom user-selected chart dimensions/metrics.
